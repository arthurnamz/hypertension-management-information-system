            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-sm-8 col-9 text-right m-b-20">
                        <a href="{{ route('urinalysis')}}" class="btn btn btn-primary btn-rounded float-right"><i class="fa fa-refresh"></i>Back to Urinalysis</a>
                    </div>
                </div>
                <div class="row">
                                ...
                                <div class="table-responsive">
                                    <table id="bootstrap-data-table-export3" class="display nowrap table table-striped table-bordered">
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$urine->pfname}} {{$urine->plname}}</td>
                                            <td>{{$urine->result}}</td>
                                            <td>{{$urine->comment}}</td>
                                            <td>{{$urine->tested_date}}</td>
                                            <td>{{$urine->efname}} {{$urine->elname}}</td>
                                            <td>
                                                <form method="POST" action=" {{ route('delete_urinalysis', $urine->id)}}" enctype="multipart/form-data">
                                                         @csrf
                                                    
                                                    <a href="{{ route('edit_urinalysis', ['id' => $urine->id] ) }}" class="btn btn-sm btn-info btn-round">Edit</a>
   ...
   <!--  <script src="js/datatables/datatables-init.js"></script> -->
    <script type="text/javascript">
        ...
        buttons: [
           {
                extend: 'copyHtml5',
                title: 'Urinalysis List',
                filename: 'Urinalysis List',
                exportOptions: {
                    columns: [ 0, 1 ,2, 3 ,4  ]
                }
            }, {
