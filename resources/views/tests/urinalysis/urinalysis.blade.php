            ...
            <div class="container-fluid">
            	<div class="row">
                    ...
                    <div class="col-sm-8 col-9 text-right m-b-20">
                        <a href="{{ route('add_urinalysis')}}" class="btn btn btn-primary btn-rounded float-right"><i class="fa fa-plus"></i> Add Urinalysis</a>
                    </div>
                    @endif
                </div>
                <div class="row">
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table id="bootstrap-data-table-export3" class="display nowrap table table-striped table-bordered">
                                        ...
                                        <tbody>
                                        @foreach($Urinalysis as $urinalysis)
                                        @if(Auth::user()->role == 'patient')
                                        <tr>
                                            <td>{{$loop -> iteration }}</td>  
                                            <td>{{$urinalysis->result}}</td>
                                            <td>{{$urinalysis->comment}}</td>
                                            <td>{{$urinalysis->tested_date}}</td>
                                            <td>{{$urinalysis->efname}} {{$urinalysis->elname}}</td>
                                        </tr>
                                        @else
                                        <tr>
                                            <td>{{$loop -> iteration }}</td>                 
                                            <td>{{$urinalysis->pfname}} {{$urinalysis->plname}}</td>
                                            <td>{{$urinalysis->pgender}}</td>
                                            <td>{{$urinalysis->pnumber}}</td>
                                            <td>{{$urinalysis->pemail}}</td>
                                            <td>{{$urinalysis->count}}</td>
                                            <td>
                                                
                                                    <a href="{{ route('view_urinalysis', ['id' => $urinalysis->id] ) }}" class="btn btn-sm btn-success btn-round">View</a> 
   ...
   <!--  <script src="js/datatables/datatables-init.js"></script> -->
    <script type="text/javascript">
        ...
        buttons: [
           {
                extend: 'copyHtml5',
                title: 'Urinalysis Test List',
                filename: 'Urinalysis Test List',
                exportOptions: {
                    columns: [ 0, 1 ,2, 3  ]
                }
            }, {
