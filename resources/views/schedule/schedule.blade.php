            ...
            <div class="container-fluid">
            	<div class="row">
                    ...
                    <div class="col-sm-8 col-9 text-right m-b-20">
                        <a href="{{ route('add_schedule')}}" class="btn btn btn-primary btn-rounded float-right"><i class="fa fa-plus"></i> Add Schedule</a>
                    </div>
                </div>
                <div class="row">
                                ...
                                <div class="table-responsive">
                                    <table id="bootstrap-data-table-export3" class="display nowrap table table-striped table-bordered">
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$slot->Employee->first_name}} {{$slot->Employee->last_name}}</td>
                                            <td>{{$slot->available_days}}</td>
                                            <td>{{$slot->start_time}} - {{$slot->end_time}}</td>
                                            <td>status</td>
                                            <td>
                                                <form method="POST" action=" {{ route('delete_schedule', $slot->id)}}" enctype="multipart/form-data">
                                                         @csrf
                                                    <!-- <a href="{{ route('patient_profile', ['id' => $slot->id] ) }}" class="btn btn-sm btn-success btn-round">View</a> -->
                                                    <a href="{{ route('edit_schedule', ['id' => $slot->id] ) }}" class="btn btn-sm btn-info btn-round">Edit</a>
